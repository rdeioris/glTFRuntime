...
void FglTFRuntimeParser::LoadStaticMeshAsync(const int32 MeshIndex, const FglTFRuntimeStaticMeshAsyncCustom& AsyncCallback, const FglTFRuntimeStaticMeshConfig& StaticMeshConfig, UStaticMeshComponent* StaticMeshComponent)
{
	// first check cache
	if (CanReadFromCache(StaticMeshConfig.CacheMode) && StaticMeshesCache.Contains(MeshIndex))
	{
		AsyncCallback.ExecuteIfBound(StaticMeshesCache[MeshIndex], StaticMeshComponent);
		return;
	}
	...
	Async(EAsyncExecution::ThreadPool, [this, StaticMeshContext, MeshIndex, AsyncCallback, StaticMeshComponent, StaticMeshConfig]()
		{
			...
			FGraphEventRef Task = FFunctionGraphTask::CreateAndDispatchWhenReady([MeshIndex, StaticMeshContext, AsyncCallback, StaticMeshComponent]()
				{
					...
					AsyncCallback.ExecuteIfBound(StaticMeshContext->StaticMesh, StaticMeshComponent);
...
UStaticMesh* FglTFRuntimeParser::LoadStaticMesh_Internal(TSharedRef<FglTFRuntimeStaticMeshContext, ESPMode::ThreadSafe> StaticMeshContext)
{
	...
	for (const FglTFRuntimeMeshLOD* LOD : LODs)
	{
		...
		for (const FglTFRuntimePrimitive& Primitive : LOD->Primitives)
		{
			...
			{
				UE_LOG(LogGLTFRuntime, Log, TEXT("Primitive is invalid"));
				continue;
			}

			if (Primitive.UVs.GetData())
			{
